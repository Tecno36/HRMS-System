...
import hrEmptyImg from '../../assets/web/hrempty1.jpg';
...
export default function HRManagement() {
  ...
  const [formData, setFormData] = useState({ name: '', email: '', phone: '', dob: '', gender: 'Male', department: '', designation: '', salary: '', bankName: '', accountNumber: '', ifscCode: '', panNumber: '' });
  const [editData, setEditData] = useState({ id: '', name: '', email: '', phone: '', dob: '', gender: 'Male', department: '', designation: '', salary: '', bankName: '', accountNumber: '', ifscCode: '', panNumber: '' });
  ...
  const fetchHRs = async () => {
    try {
      ...
      const res = await axios.get('/hr/hr-list', {
        headers: { Authorization: `Bearer ${token}` }
      });
      setHrs(res.data);
    } catch (err) {
      setError('Failed to fetch HR list');
    }
  };
  ...
  const handleAddHR = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.post('/hr/create-hr', formData, {
        headers: { Authorization: `Bearer ${token}` }
      });
      setSuccess('HR successfully added! Credentials sent to email.');
      setFormData({ name: '', email: '', phone: '', dob: '', gender: 'Male', department: '', designation: '', salary: '', bankName: '', accountNumber: '', ifscCode: '', panNumber: '' });
      setFieldErrors({});
      setShowModal(false);
      fetchHRs();
    } catch (err) {
      setError(err.response?.data?.message || 'Failed to add HR');
    } finally {
      setLoading(false);
    }
  };
  ...
  const openEditModal = (hr) => {
    setEditData({ 
      id: hr._id, 
      name: hr.name || '', 
      email: hr.userId?.email || hr.email || '', 
      phone: hr.phone || '', 
      dob: hr.dob ? new Date(hr.dob).toISOString().split('T')[0] : '',
  ...
  const handleEditHR = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.put(`/hr/update-hr/${editData.id}`, editData, {
        headers: { Authorization: `Bearer ${token}` }
      });
      setSuccess('HR details updated successfully.');
      setShowEditModal(false);
      fetchHRs();
    } catch (err) {
      setError(err.response?.data?.message || 'Failed to update HR');
    } finally {
      setLoading(false);
    }
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`/hr/delete-hr/${deleteId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      setSuccess('HR access revoked successfully.');
      setShowDeleteDialog(false);
      setDeleteId(null);
      fetchHRs();
    } catch (err) {
      setError(err.response?.data?.message || 'Failed to delete HR');
    } finally {
      setLoading(false);
    }
  };
  ...
  const filteredHRs = hrs.filter(hr => 
    ...
    (hr.userId?.email || hr.email || '').toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const currentItems = filteredHRs.slice(startIndex, startIndex + itemsPerPage);
  ...
  return (
    <div className="p-6 md:p-8 max-w-[1400px] mx-auto space-y-8 relative">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
        <div>
          <h1 className="text-2xl font-black text-gray-900 tracking-tight">HR Management</h1>
          <p className="text-sm text-gray-500 mt-1 font-medium">Manage all department HR administrators and access.</p>
        </div>
        <div className="flex flex-col sm:flex-row items-center gap-3 w-full sm:w-auto">
          ...
          <button 
            ...
            className="group w-full sm:w-auto flex items-center justify-center gap-2 px-6 py-3 bg-[#101a3d] hover:bg-indigo-600 text-white font-bold rounded-xl shadow-xl shadow-indigo-500/20 transition-all duration-300 active:scale-95"
          >
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
                ...
                <th className="px-6 py-5 rounded-tl-3xl">HR Name</th>
            ...
            <tbody className="divide-y divide-gray-50 text-sm">
                ...
                currentItems.map((hr) => (
                    <tr key={hr._id} className="hover:bg-gray-50/50 transition-colors group">
                    <td className="px-6 py-4">
                        ...
                        <div>
                            <p className="font-bold text-gray-900">{hr.name}</p>
                            <p className="text-xs font-bold text-gray-400 mt-0.5">{hr.employeeId || 'Pending ID'}</p>
                        </div>
                        </div>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4 text-center">
                        ...
                        <button 
                            onClick={() => { setDeleteId(hr._id); setShowDeleteDialog(true); }}
                    ...
                    <td colSpan="6" className="px-2 py-12 text-center">
                      <div className="flex flex-col items-center justify-center">
                        <img src={hrEmptyImg} alt="No HR Found" className="w-64 h-64 object-contain" />
                        <p className="mt-4 text-sm font-medium text-gray-500">No HR Administrators found.</p>
                      </div>
                   </td>
                </tr>
                )}
        ...
        {totalPages > 1 && (
          <div className="flex flex-col sm:flex-row justify-between items-center px-6 py-4 border-t border-gray-100 bg-gray-50/50 gap-4">
            <span className="text-sm font-medium text-gray-500">
              Showing {startIndex + 1} to {Math.min(startIndex + itemsPerPage, filteredHRs.length)} of {filteredHRs.length} entries
            </span>
            <div className="flex items-center gap-2">
              <button 
                onClick={() => setCurrentPage(p => Math.max(1, p - 1))}
                disabled={currentPage === 1}
                className="px-4 py-2 border border-gray-200 rounded-xl text-sm font-bold text-gray-600 bg-white hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
                Previous
              </button>
              <div className="flex items-center gap-1">
                {[...Array(totalPages)].map((_, i) => (
                  <button
                    ...
                    className={`w-9 h-9 rounded-xl text-sm font-bold transition-colors ${currentPage === i + 1 ? 'bg-indigo-600 text-white shadow-md' : 'text-gray-600 hover:bg-gray-100'}`}
                  >
                    {i + 1}
                  </button>
                ))}
        ...
        <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4 sm:p-6 overflow-hidden">
          ...
          <div className="relative w-full max-w-4xl max-h-[90vh] flex flex-col bg-white rounded-[2rem] shadow-2xl border border-white overflow-hidden transform transition-all">
              ...
              <div className="flex justify-between items-center">
                <div>
                  <h3 className="text-2xl font-black text-gray-900 tracking-tight">
                    {showEditModal ? 'Edit HR Profile' : 'Create HR Profile'}
                  </h3>
                  <p className="text-sm text-gray-500 font-medium mt-1">
                    {showEditModal ? 'Update the details for this HR admin.' : 'Enter personal, work, and bank details for the new HR.'}
                  </p>
                </div>
                <button 
                  onClick={() => {
            ...
            <div className="overflow-y-auto p-8 relative z-10">
              <form onSubmit={showEditModal ? handleEditHR : handleAddHR} noValidate className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                  ...
                  <div className="space-y-2">
                    ...
                    <select 
                      name="department" 
                      value={showEditModal ? editData.department : formData.department} 
                      onChange={(e) => handleChange(e, showEditModal)} 
                      className={`w-full px-4 py-3.5 bg-gray-50/50 border rounded-2xl text-sm font-medium text-gray-900 focus:outline-none focus:ring-4 focus:bg-white transition-all cursor-pointer ${fieldErrors.department ? 'border-red-400 focus:ring-red-500/10 focus:border-red-500' : 'border-gray-200 focus:ring-indigo-500/10 focus:border-indigo-500'}`}
                    >
            ...
            <div className="flex-shrink-0 flex items-center justify-end gap-4 p-6 border-t border-gray-100 bg-gray-50/80">
              ...
              <button 
                type="button"
                onClick={showEditModal ? handleEditHR : handleAddHR}
                disabled={loading} 
              ...
              >
                {loading ? (
                  ...
                  <span className="flex items-center gap-2">
                    {showEditModal ? 'Save Changes' : 'Create HR Profile'}
        ...
        <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4 sm:p-6">
          ...
          <div className="relative w-full max-w-md bg-white rounded-3xl shadow-2xl p-8 transform transition-all text-center">
            ...
            <p className="text-sm text-gray-500 mb-8 leading-relaxed">
              This action will revoke access for this HR Administrator. 
              (They won't be able to log in, but their added employees will remain safe).
            </p>
